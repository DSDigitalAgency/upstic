    ...
    debounce(async (currentSearch: string) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          const pendingJobs = response.data.items.filter(job => job.status.toUpperCase() === 'PENDING');
          setJobs(pendingJobs);
        } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Review and manage job postings that are not yet active</p>
        </div>
        <button 
